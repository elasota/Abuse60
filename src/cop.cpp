...
void *cop_mover(int xm, int ym, int but)
{
	...
	else
	{
		...
		{
			// The terminal velocity code above has a small amount of slippage:
			// If fyacel < 56 and (fyvel + fyacel) < 256, then velocity will decrease by 1 for the frame.
			// If fyacel >= 56 and (fyvel + fyacel) >= 312, then velocity will increase by 1 for the frame.
			// The net change is 0.19384765625 velocity per frame.
			// 
			// Decrementing the fractional velocity by 48 produces roughly a similar rate of slippage.
			// Additionally, due to subframe acceleration, terminal velocity usually doesn't kick in until the
			// velocity is about 12, even though the condition is 10.
			if (o->yvel() > 12)
			{
				o->set_yacel(0);

				int32_t yv = (o->yvel() << 8) + o->fyvel();
				yv -= 48;
				o->set_yvel(yv >> 8);
				o->set_fyvel(yv & 255);
			}
