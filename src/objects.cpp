...
int game_object::tick()      // returns blocked status
{
	...
	//
	// This means:
	// Accel(t) = 25 / 32 * t + InitialAccel
	// Vel(t) = 25 / 64 * t + 25 / 64 * t ^ 2 + (InitialAccel * t) + InitialVel
	// Coord(t) = 25 / 96 * t + 25 / 64 * t ^ 2 + 25 / 192 * t ^ 3 + (InitialAccel / 2) * (t + t ^ 2) + (InitialVel) * t + InitialCoord
	//
	// At 1/4 stepping:
	// Accel' = 25/128 + InitialAccel
	// Vel' = 25/256 + 25/1024 + InitialAccel/4 + Vel
	// Coord' = 25/384 + 25/1024 + 25/12288 + InitialAccel*5/32 + InitialVel/4 + Coord
	//
	// Without gravity:
	// Vel' = Vel + Accel
	// Coord' = Coord + Vel'
	//
	// Means:
	// Vel(t) = (InitialAccel * t) + InitialVel
	// Coord(t) = (InitialAccel / 2) * (t + t^2) + (InitialVel * t) + InitialCoord

	int32_t integrated_accel_x_to_vel = 0;
	int32_t integrated_accel_y_to_vel = 0;
	int32_t integrated_accel_x = 0;
	int32_t integrated_accel_y = 0;
	int32_t initial_accel_x = static_cast<int32_t>(fxacel()) + (xacel() << 8);
	int32_t initial_accel_y = static_cast<int32_t>(fyacel()) + (yacel() << 8);

	int32_t init_yvel = yvel();
	int32_t init_fyvel = fyvel();
	int32_t init_yacel = yacel();
	int32_t init_fyacel = fyacel();

	int32_t accelx32 = initial_accel_x;
	int32_t accely32 = initial_accel_y;
	...
	if (gravity() && !floating())
	{
		...
		{
			int32_t new_yacel = initial_accel_y;
			if (yacel() >= 0)
			{
				integrated_accel_y_to_vel = 31;
				integrated_accel_y = 23;
				new_yacel += 50;
			}
			else
			{
			...
			set_yacel(new_yacel >> 8);
			set_fyacel(new_yacel & 255);
		}
	}

	// first let's move the guy acording to his physics
	int32_t initial_vel_x = 0;
	int32_t initial_vel_y = 0;
	if (!fps60)
	{
	...
	{
		int32_t vx32 = static_cast<int32_t>(fxvel()) + (xvel() << 8);
		int32_t vy32 = static_cast<int32_t>(fyvel()) + (yvel() << 8);

		initial_vel_x = vx32;
		initial_vel_y = vy32;

		vx32 += integrated_accel_x_to_vel + initial_accel_x / 4;
		vy32 += integrated_accel_y_to_vel + initial_accel_y / 4;
	...
	if (old_vx || old_vy)
	{
		...
		if (!fps60)
		{
			...
			h = try_move(x, y, xv, yv, 1 | up);       // now find out what velocity is safe to use
			set_xvel(xv);
			set_yvel(yv);
			x += xv;
			y += yv;
		...
		if (xv != try_xv || yv != try_yv)     // he collided with something
		{
